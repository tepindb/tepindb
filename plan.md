...
4. **Tooling** — ✅ `tepin mcp` server, CLI search, npx packaging (`npm/`: tepindb + tepindb-<platform> packages, published with provenance from release.yml). Published: crates 0.4.0, npm tepindb 0.4.0 (+ darwin/linux platform pkgs). Pending: npm win32-x64 (spam-flagged, awaiting npm support).
...
7. **Engram dossier (0.4.0)** — ✅ released 2026-07-21 (crates + npm + GitHub binaries/SBOM; npm win32 still spam-blocked). The first production integrator's requirements, all in one release: **serving** (multi-process reads via in-driver IPC, `docs/serving.md`, phase 1 full — no write-forwarding), **`tepin migrate`** (infrastructure + fixtures + CI + stability policy, `docs/format-stability.md`; no format break yet), embedder **unpin/reset** (`reset_embedder`), purposed-but-empty reads return `Ok(empty)`, native **upsert** (incl. batch + CLI + MCP), optional **unique** secondary indexes, open-with-retry/backoff (`Db::options().retry_for`).
8. **Drivers** — Go + TS ✅ built in-tree (unreleased): one C ABI (`tepin-ffi` → `libtepin`), thin in-process shells — `tepin-go` (pure Go, purego dlopen, SHA-256-pinned lib download from GitHub releases) and `import {open} from "tepindb"` (napi addon in the platform packages). Same verbs as CLI/MCP via the shared `tepin_core::ops` dispatch; errors carry a CLI repro; `TEPIN_TRACE=1` traces every op in every language. Design: `docs/drivers.md`. CI runs both e2e on all tier-1 targets; release.yml attaches full `libtepin`/addon assets and tags `tepin-go/vX.Y.Z` with committed pins. Ships with the next release. Python still open.
